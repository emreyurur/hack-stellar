...
export class RiskController {
  ...
  @ApiResponse({ status: 200, description: 'Risk score details', type: RiskResponseDto })
  ...
  async getPoolRisk(@Param('id') id: string): Promise<RiskResponseDto> {
    ...
    if (!risk) {
      try {
        ...
        throw new NotFoundException(`Risk data for pool ${id} not found and could not be calculated: ${e.message}`);
      }
    }
  ...
  @Get('level/:level')
  @ApiOperation({ summary: 'Get pools filtered by a specific risk level (LOW, MEDIUM, HIGH)' })
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'limit', required: false, type: Number })
  @ApiResponse({ status: 200, description: 'Paginated list of pools with their risk scores' })
